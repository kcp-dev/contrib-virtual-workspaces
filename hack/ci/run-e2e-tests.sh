...
# two images under test. The tests own everything below that: each creates a namespace and deploys
# its own kcp into it, so the setup here is done once no matter how many tests run.
#
...
# tests log the pods of both virtual workspaces themselves; what they cannot show is the state of
# the objects kcp-operator was asked to reconcile, or why it did not.
dump_diagnostics() {
	echo
