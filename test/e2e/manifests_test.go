...
// also catches the mistake the templates are most prone to: a placeholder that no longer matches a
// field of templateData, which `missingkey=error` turns into a render failure here.
func TestManifestsRender(t *testing.T) {
	data := templateData{
		...
		AccessImageRepository:      "localhost/access-vw",
		AccessImageTag:             "e2e",
		MCPImageRepository:         "localhost/mcp-vw",
		MCPImageTag:                "e2e",
	...
	manifests := []string{
		"etcd.yaml.tmpl",
		"kcp.yaml.tmpl",
		"access-rbac.yaml.tmpl",
		"access-virtualworkspace.yaml.tmpl",
		"mcp-virtualworkspace.yaml.tmpl",
		"consumer.yaml.tmpl",
	}
